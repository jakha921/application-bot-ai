...
export const BotSelector = ({ bots, currentBotId, onBotChange }: BotSelectorProps) => {
  const navigate = useNavigate();

  return (
    <div className="flex flex-col sm:flex-row items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-xl shadow-md mb-6">
      <div className="flex items-center mb-4 sm:mb-0 w-full sm:w-auto">
        ...
        >
          {bots.length > 0 ? (
            ...
            <option>Нет доступных ботов</option>
          )}
        </select>
      </div>
      <button
